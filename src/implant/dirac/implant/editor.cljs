...
(ns dirac.implant.editor
  ...
  (:require
    ...
    [dirac.implant.state :refer [state
                                 empty-editor-state]]
    [dirac.implant.editor-support :refer [update-cursor!
                                          fix-text!
                                          cm-key
                                          IEditor
                                          get-prev-state
...
(defn on-change
  ...
  (when (not= "setValue" (.-origin change))
    #_(record-change! cm {:change (parse-change change)})
...
(defn on-cursor-activity
  ...
  (when-not (frame-updated? cm)
    #_(record-change! cm {:selections (parse-selections (.listSelections cm))})
    (fix-text! cm))
   ...
   (when-not (get @state key-)
     (let [element-id (.-id element)
       ...
       (specify! cm
                 ...
                 #_(record-change! [this new-thing]
