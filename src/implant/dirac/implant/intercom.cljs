...
(defn check-runtime-version! [runtime-version]
  (let [our-version implant-version/version]
    (when-not (= runtime-version our-version)
...
(defn go-wait-for-client-disconnection []
...
(defn go-connect-to-weasel-server! [url]
  (go
    (if-some [runtime-config (<! (eval/go-get-runtime-config))]
      (do
        (let [runtime-tag (<! (eval/go-get-runtime-tag))
...
(defn handle-tunnel-error [url _client _event]
...
(defn connect-to-nrepl-tunnel-server! [url verbose? auto-reconnect? response-timeout]
  ...
  (when-not *last-connection-url*
    ...
    (try
      (let [tunnel-options (utils/remove-nil-values {:on-error          (partial handle-tunnel-error url)
...
(defn go-start-repl! []
  (go
    (let [runtime-version (<! (eval/go-get-runtime-version))]
      (check-runtime-version! runtime-version)
      (let [repl-api-version (<! (eval/go-get-runtime-repl-api-version))]
...
(defn go-react-on-global-object-cleared! []
  ...
  (go
    ...
    (<! (go-wait-for-client-disconnection))
...
(defmethod nrepl-tunnel-client/go-process-message :bootstrap [_client message]
  ...
  (go
    (let [runtime-tag (<! (eval/go-get-runtime-tag))
          runtime-config (<! (eval/go-get-runtime-config))
          bootstrap-message (make-bootstrap-message runtime-config runtime-tag)
          responses-chan (nrepl-tunnel-client/go-tunnel-message-with-responses! bootstrap-message)]
      (loop []
...
(defmethod nrepl-tunnel-client/go-process-message :bootstrap-info [_client message]
  (let [{:keys [weasel-url]} message]
    (assert weasel-url (str "expected :weasel-url in :bootstrap-info message" message))
    (go-connect-to-weasel-server! weasel-url))
  (go))
...
(defn handle-forwarded-eval-message! [forwarded-message job-id]
  ...
  (if-some [code (:code forwarded-message)]
...
(defmethod nrepl-tunnel-client/go-process-message :handle-forwarded-nrepl-message [_client message]
...
(defn go-send-devtools-request! [request payload]
  (assert (repl-ready?))
  (let [nrepl-message (make-devtools-request-message request payload)
        responses-chan (nrepl-tunnel-client/go-tunnel-message-with-responses! nrepl-message)]
    responses-chan))
